...
export default {
  ...
  created () {
    ...
    // let userMessage = this.$store.state.user.access
    // if (userMessage) {
    //   userMessage = JSON.parse(userMessage)
    //   this.$globalFunction.connectSocket('ws://10.128.208.191:8080/ws/')
    // }
  },
  computed: {
    ...mapGetters([ // vuex的辅助函数：映射为this.$store.getters.xxxx
      'errorCount'
    ]),
    tagNavList () {
  ...
  methods: {
    ...mapMutations([ // vuex的辅助函数：映射为 this.$store.commit("xxx",n) 同步
      'setBreadCrumb',
      'setTagNavList',
      'addTag',
      'setLocal',
    ...
    ...mapActions([ // vuex的辅助函数：映射为 this.$store.dispatch("xxx",n); 异步
      'handleLogin',
  ...
  watch: {
    '$route' (newRoute) {
      ...
      this.getUnreadMessageCount() //! !!!!!!!!!!在这里加了东西
