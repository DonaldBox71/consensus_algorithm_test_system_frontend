<template>
  <div class="vue-city-selector"
    ...
    <input v-show="!noInput"
           ...
           ref="cityParkInput"
           id="help_bind_show">
    <label for="help_bind_show">(按首字母检索)</label>
    <!--小区选择-->
    <div class="vcp-panel"
         v-show="cityPanelIsShow">
      <h5>请选择 <span class="clear-citypanel"
              @click="clearCityPanel">清空</span></h5>
      <ul class="vcp-tags">
        <li :class="{'z-on': item==tagKey}"
...
export default {
  ...
  methods: {
    ...
    // 鼠标离开小区选择区域时超过一定时间，关闭小区面板
    endChoice (e) {
      let that = this

      let inputEle = that.$refs.cityParkInput
      if (e.type === 'mouseout') {
